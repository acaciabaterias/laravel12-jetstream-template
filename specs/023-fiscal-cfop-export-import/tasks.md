...
- [ ] T037 [P] [US1] Add feature coverage for interstate fiscal scenario lookup with origin/destination and partner context in `tests/Feature/PlatformFiscalInterstateLookupTest.php`
- [ ] T038 [P] [US2] Add unit coverage for tax profile validation rules including NCM, CST/CSOSN and interstate exceptions in `tests/Unit/PlatformFiscalTaxProfileRulesTest.php`
- [ ] T039 [P] [US2] Add feature coverage for publication blocking when tax profile metadata is incomplete in `tests/Feature/PlatformFiscalTaxProfilePublicationTest.php`
- [ ] T040 [US1] Extend central fiscal schema and models for material tax profile data in `database/migrations/central/` and `app/Models/`
- [ ] T041 [US1] Implement tax profile resolution services for interstate and international scenarios in `app/Services/Fiscal/`
- [ ] T042 [US2] Strengthen publication guardrails for NCM/CST/CSOSN, regime compatibility and interstate tax context in `app/Services/Fiscal/` and `app/Http/Requests/Admin/`
- [ ] T043 [US1] Expose enriched fiscal resolution contract for module `009` consumers in `app/Services/Fiscal/` and `app/Http/Controllers/Admin/`
- [ ] T044 [US3] Extend inspection and rollback evidence to include tax profile gaps and tributary exceptions in `app/Services/Fiscal/`, `app/Livewire/Admin/` and `resources/views/livewire/admin/`
- [ ] T045 [US3] Update runbook and quickstart evidence for interstate taxation and material fiscal payload validation in `GO_LIVE_RUNBOOK.md` and `specs/023-fiscal-cfop-export-import/quickstart.md`
