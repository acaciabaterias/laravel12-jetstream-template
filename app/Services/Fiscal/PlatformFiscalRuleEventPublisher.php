...
class PlatformFiscalRuleEventPublisher
{
    ...
    {
        if (! config('platform_fiscal_rules.events.publish_to_backbone', true)) {
            return;
        }
        ...
        $this->integrationStorageManager->using('central', function () use ($eventType, $payload, $consumers): void {
            $this->eventPublisher->publish(
                ...
                idempotencyKey: sprintf('platform-fiscal-rules:%s:%s', strtolower($eventType), sha1((string) json_encode($payload))),
                correlationId: (string) Str::uuid(),
                eventVersion: 'v1',
                originContext: 'platform-fiscal-rules',
                metadata: [
                    'consumers' => $consumers,
                    'transport_kind' => 'broker',
                    'target' => 'broker:platform-fiscal-rules',
                ],
            );
        });
    }
