...
class PlatformLocalizationEventPublisher
{
    ...
    {
        if (! config('platform_localization.events.publish_to_backbone', true)) {
            return;
        }
        ...
        $this->integrationStorageManager->using('central', function () use ($eventType, $payload, $consumers): void {
            $this->eventPublisher->publish(
                ...
                idempotencyKey: sprintf('platform-localization:%s:%s', strtolower($eventType), sha1((string) json_encode($payload))),
                correlationId: (string) Str::uuid(),
                eventVersion: 'v1',
                originContext: 'platform-localization',
                metadata: [
                    'consumers' => $consumers,
                    'transport_kind' => 'broker',
                    'target' => 'broker:platform-localization',
                ],
            );
        });
    }
